...
class base_HPO():
    ...
    def getFinetunedCondidateConifg(self, sample_num=1e3, finetuneOtherHP=False):
        ...
        # print(f'==> get {sample_num} FinetunedCondidateConifg from {len(self.observed_config_dict)} observed configs')
    ...
    def runTrials(self, maxTrials, sample_num, explore_trials=1000, reduce_space_trials=1000, meta_feature=None, 
                    start_candidate=None, start_candidate_dataset=None):
        ...
        for trial in range(int(maxTrials)):
            ...
            if start_candidate != None and (trial+1) <= len(start_candidate):
                next_config   = start_candidate[trial]
                cfg_pred_y    = -1

            else:
                if trial < explore_trials:
                    candidate_config = self.getCondidateConifg(sample_num=sample_num) # exploration
                else:
                    candidate_config = self.getFinetunedCondidateConifg() # exploitation

                candidate_config_X = self.changeConfigToArray(candidate_config)

                if type(meta_feature) is np.ndarray:
                    feature_matrix     = np.tile(meta_feature, (candidate_config_X.shape[0], 1))
                    # print(candidate_config_X.shape, feature_matrix.shape)
                    candidate_config_X = np.concatenate((candidate_config_X, feature_matrix), axis=1)
            ...
            try:
                real_y = self.obj_function(copy.deepcopy(next_config))
            except RuntimeError as exception:
                # print('==> Out of memory in obj_function.')
                # real_y = -1

                if "out of memory" in str(exception) or "CUDNN_STATUS" in str(exception):
                    print('==> Out of memory in obj_function.')
                    real_y = -1
                else:
                    print('==> RuntimeError in obj_function.')
                    print(str(exception))
                    exit()
            print(f'==> config:{next_config}, MRR={real_y}')
